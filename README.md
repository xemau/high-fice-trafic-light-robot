...
The prepared `TEMPROBOT` folder contains 150 randomized reward excerpts, numbered continuously from `0001.mp3` through `0150.mp3`. Its manifest records the title, artist, source URL, excerpt start, file size and SHA-256 hash for every track. Each file is already approximately 30.3 seconds long; firmware still enforces the 30-second reward deadline. See [the SD preparation record](docs/sd-card-preparation.md) for verification details.
...
| Debounce | 30 ms |
| Reward tracks | 1–150 (random, no immediate repeat) |
...
| Audio stops working after one reward | Capture the boot build timestamp, `[AUDIO TX]` and error lines through the second press, then send `status`. `PLAY N queued` means accepted by firmware; `cmd=0x12 arg=N` means written to UART, neither proves sound. If `audio=failed`, send `retry`, wait for `[OK] YX5200`, then try several `play N` commands across the 1–150 range and record which work |
