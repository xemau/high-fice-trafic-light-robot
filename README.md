...
| --- | --- | --- | --- |
| `1` | FULL | Sensor, LEDs, UART2 | Automatic red → yellow → green → high-five → animation → yellow+green warning → red |
...
7. **SEQUENCE TEST (`5`).** Verify red (3 s), yellow (1 s), green (indefinite), then `highfive` → reward animation → yellow+green at 26 s → stop and red at 30 s. Confirm the next green waits for a new press. This mode requires no physical peripheral and does not play actual audio or drive LEDs.
