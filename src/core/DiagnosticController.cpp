...
void DiagnosticController::begin(AppMode mode) {
    ...
    log_.log(message);
    if (lightsMode()) log_.log(lights().begin() ? "[OK] LEDs (driver initialized)" : "[ERROR] LEDs initialization failed");
    if (robotMode() || mode_ == AppMode::SensorTest) {
        log_.log(sensor().begin() ? "[OK] sensor" : "[ERROR] sensor initialization failed");
